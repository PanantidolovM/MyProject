...
public class EmployeeApiTester
{
    ...
    private static readonly HttpClient _client = new HttpClient{BaseAddress = new Uri("http://localhost:5048/")};

    // 1️⃣ Tạo nhân viên mới
    public static async Task TestCreateEmployee()
    {
        Console.WriteLine("📡 Đang gửi request POST /api/employees/add...");
        ...
        var response = await _client.PostAsJsonAsync("api/employees/add", newEmployee);
    ...
    public static async Task TestGetEmployeeDetails()
    {
        var employeeId = 0; // ID của nhân viên cần lấy thông tin
        var response = await _client.GetAsync($"api/employees/get/{employeeId}");
        if (response.IsSuccessStatusCode)
        {
    ...
    public static async Task TestUpdateEmployee()
    {
        ...
        var response = await _client.PutAsJsonAsync("api/employees/update", updatedEmployee);
        if (response.IsSuccessStatusCode)
        {
    ...
    public static async Task TestDeleteEmployee()
    {
        var employeeId = 1; // ID của nhân viên cần xóa
        var response = await _client.DeleteAsync($"api/employees/delete/{employeeId}");
        if (response.IsSuccessStatusCode)
        {
