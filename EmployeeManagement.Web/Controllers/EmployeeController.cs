...
public class EmployeeController(IEmployeeAsyncAppService employeeAsyncAppService, ILogger<EmployeeController> logger) : ControllerBase
{
    ...
    public async Task<IActionResult> AddEmployee([FromBody] DtoEmployee employeeDto)
    {
        try
        {
            _logger.LogInformation("Controller: Adding new employee: {EmployeeJson}", JsonSerializer.Serialize(employeeDto, new JsonSerializerOptions { WriteIndented = true }));
            await _employeeAsyncAppService.AddEmployeeAsync(employeeDto);
            _logger.LogInformation("✅ Employee added successfully: {EmployeeJson}", JsonSerializer.Serialize(employeeDto, new JsonSerializerOptions { WriteIndented = true }));
